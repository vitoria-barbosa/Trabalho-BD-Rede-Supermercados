...
CREATE OR REPLACE FUNCTION REALIZAR_VENDA(P_CPF_CLI TEXT, P_ID_MERC TEXT, P_ID_FUNC TEXT, P_TIPO_PG TEXT, P_ITENS JSONB) RETURNS VOID AS $$
DECLARE
    ID_VENDA INT;
    ITEM RECORD;
BEGIN
    ID_VENDA := INSERIR('VENDA', P_CPF_CLI, P_ID_FUNC, P_TIPO_PG);

    FOR ITEM IN SELECT * FROM jsonb_to_recordset(P_ITENS) AS x(NOME TEXT, QTD INT)
    LOOP
        PERFORM INSERIR('ITEM_VENDA', P_ID_MERC::TEXT, UPPER(ITEM.NOME), ID_VENDA::TEXT, ITEM.QTD::TEXT);
    END LOOP;
...
SELECT REALIZAR_VENDA('987', '1', '7', 'DÉBITO', 
'[{"nome": "FARINHA DE TRIGO", "qtd": 1}, {"nome": "macarrão", "qtd": 2}]'::jsonb)
